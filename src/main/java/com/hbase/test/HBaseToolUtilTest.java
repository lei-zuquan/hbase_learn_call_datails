...
public class HBaseToolUtilTest {
	...
	public static void main(String[] args) throws Exception{
		...
		com.hbase.util.HBaseToolUtil.createNamespace(NAME_SPACE);
		com.hbase.util.HBaseToolUtil.createTable(TABLE_NAME, REGION_COUNT, MAX_VERSIONS, TIME_TO_LIVE, COL_FAMILY);
		...
		//singleThreadDoHbase();
		multiThreadDoHbaseByHBaseToolUtil();
	...
	private static void multiThreadDoHbaseByHBaseToolUtil() throws Exception{
		...
		for (int i = startNo; i < threadCount; i++) {
			...
			MyThreadWriteDataToHbaseByHBaseToolUtil childThread = new MyThreadWriteDataToHbaseByHBaseToolUtil(threadName);
	...
	public static void insertOneDataToHbaseByHBaseToolUtil(String prefix){
		...
		for (int j = 0; j < 100; j++) {
			...
			pnum = com.hbase.util.HBaseToolUtil.reverseRowkey(pnum);
			int regionNum = com.hbase.util.HBaseToolUtil.genRegionNum(pnum, REGION_COUNT);
	...
	private static void singleThreadDoHbase() throws Exception{
		...
		// 插入测试数据，这里不建议使用insertToDB1接口
		com.hbase.util.HBaseToolUtil.truncateTable(TABLE_NAME);
		...
		Result result = com.hbase.util.HBaseToolUtil.getOneRow(TABLE_NAME, rowKey);
		if (!result.isEmpty()){
			byte[] family = COL_FAMILY.getBytes();
...
//
//
		com.hbase.util.HBaseToolUtil.flush(TABLE_NAME);
...
//		testRowKeyLength();
	...
	private static void insertToDB1() throws Exception{
		...
		com.hbase.util.HBaseToolUtil.savePutList(list, TABLE_NAME);
		com.hbase.util.HBaseToolUtil.savePut(put, TABLE_NAME);
		
		com.hbase.util.HBaseToolUtil.insert(TABLE_NAME, "testrow", COL_FAMILY, "age", "35") ;
		com.hbase.util.HBaseToolUtil.insert(TABLE_NAME, "testrow", COL_FAMILY, "cardid", "12312312335") ;
		com.hbase.util.HBaseToolUtil.insert(TABLE_NAME, "testrow", COL_FAMILY, "tel", "13512312345") ;
		
		com.hbase.util.HBaseToolUtil.insert(TABLE_NAME, "2014-01-01", COL_FAMILY, "age", "99") ;
		com.hbase.util.HBaseToolUtil.insert(TABLE_NAME, "2014-01-01", COL_FAMILY, "cardid", "11312312335") ;
		com.hbase.util.HBaseToolUtil.insert(TABLE_NAME, "2014-02-01", COL_FAMILY, "tel", "11512312345") ;
		
		List<Result> resultList = com.hbase.util.HBaseToolUtil.getRows(TABLE_NAME, "2014-01", COL_FAMILY, "age") ;
		for(Result result : resultList){
			com.hbase.util.HBaseToolUtil.printRowRecord(result);
		}
		
		Result result = com.hbase.util.HBaseToolUtil.getOneRow(TABLE_NAME, "2014-01-01");
		com.hbase.util.HBaseToolUtil.printRowRecord(result);
	}
...
//	private static Random r = new Random();
	...
	private static void insertPhoneNumToDB() throws Exception{
		...
		for (int i = 0; i < 100; i++) {
			...
			if (puts.size() >= 100){
				com.hbase.util.HBaseToolUtil.savePutList(puts, TABLE_NAME);
	...
	public static void scanPhoneRecord(String phoneNum, String startMonth, String endMonth) throws Exception {
		...
		int regionNum = com.hbase.util.HBaseToolUtil.genRegionNum(phoneNum, REGION_COUNT);
		...
		List<Result> results = com.hbase.util.HBaseToolUtil.getRows(TABLE_NAME, COL_FAMILY, startRowKey, stopRowKey);
		byte[] family = COL_FAMILY.getBytes();
		for (Result rs : results) {
			System.out.print(com.hbase.util.HBaseToolUtil.getColValue(rs, family, CF1_DNUM));
			System.out.print(" - " + com.hbase.util.HBaseToolUtil.getColValue(rs, family, CF1_DATE));
	...
	public static void scanDBPhoneNumByFilter(String phoneNum) throws Exception {
		...
		ResultScanner rss = com.hbase.util.HBaseToolUtil.scanRowByFilterList(TABLE_NAME, list);
		for (Result rs : rss) {
			System.out.print(com.hbase.util.HBaseToolUtil.getColValue(rs, family, CF1_DNUM.getBytes()));
			System.out.print(" - " + com.hbase.util.HBaseToolUtil.getColValue(rs, family, CF1_DATE.getBytes()));
			System.out.print(" - " + com.hbase.util.HBaseToolUtil.getColValue(rs, family, CF1_TYPE.getBytes()));
			System.out.println(" - " + com.hbase.util.HBaseToolUtil.getColValue(rs, family, CF1_LENGTH.getBytes()));
		}
		rss.close();
